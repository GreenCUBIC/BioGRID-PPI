...
function recalculate_crop( v_score, v_labels, Data, k )
  ...
  pfile = io.open(opt.dataDir..'Results/prediction-'..saveName..'_fold-'..k..'.txt', 'w')
...
function cv_files_exist( file, k_fold )
  all_found = true
  for k=1, k_fold do
    if file_exists(file..'_train_fold-'..k..'_labels.dat') == false then all_found = false end
    if file_exists(file..'_test_fold-'..k..'_labels.dat') == false then all_found = false end
    if file_exists(file..'_train_fold-'..k..'_profile_crop_'..opt.cropLength..'.t7') == false then all_found = false end
    if file_exists(file..'_test_fold-'..k..'_profile_crop_'..opt.cropLength..'.t7') == false then all_found = false end
    if file_exists(file..'_train_fold-'..k..'_number_crop_'..opt.cropLength..'.t7') == false then all_found = false end
    if file_exists(file..'_test_fold-'..k..'_number_crop_'..opt.cropLength..'.t7') == false then all_found = false end
  return all_found
  end
end
--******************** MAIN-RUN *******************************************
